...
- Реализация: добавлен `worlds/documentation-world/loader.js`, который читает `seed.nodes.json` + `seed.edges.json`, маппит `title → label` и собирает GraphModel для engine без изменения pipeline.[[1]](OPUS_TASK__DOC_WORLD_LOAD__LOADER_AND_SMOKE_TESTS%20bd2a7eca62334c16a8ce9c7dc2667ff6.md)
- Верификация: добавлен smoke‑тест загрузки documentation-world (путь в исходных отчётах может быть историческим для monorepo snapshot; в текущем `utemix-org/meaning-engine` тесты и демо привязаны к `src/*`, `operators/*`, `worlds/*`).
    ...
    - Regression: 562 tests passed, 0 failed.[[1]](OPUS_TASK__DOC_WORLD_LOAD__LOADER_AND_SMOKE_TESTS%20bd2a7eca62334c16a8ce9c7dc2667ff6.md)
- Ограничение: пока не подключено в dev UI как selectable world; только loader + tests.[[1]](OPUS_TASK__DOC_WORLD_LOAD__LOADER_AND_SMOKE_TESTS%20bd2a7eca62334c16a8ce9c7dc2667ff6.md)

### 2026-03-13 — Documentation World Analysis Operators (SUCCESS)

- Реализация: добавлены analysis operators для doc-world (`runAnalysis.js`) и артефакты результата (`analysis.json`, `analysis.md`) + тест `documentationWorldAnalysis.test.js`.[[2]](OPUS_TASK__DOC_WORLD_ANALYSIS_OPERATORS__CENTRALIT%209fe3882fa45441db93e18f4b0834565c.md)
- Результаты:
    - Centrality: **Focus** — абсолютный центр (max betweenness), **Workbench** — второй центр → подтверждает модель Focus-driven и Workbench-as-operator.[[2]](OPUS_TASK__DOC_WORLD_ANALYSIS_OPERATORS__CENTRALIT%209fe3882fa45441db93e18f4b0834565c.md)
    - Bridges: самый слабый мост **Context ↔ SYSTEM_OVERVIEW** → сигнал укрепить связность между overview и concept-слоем.[[2]](OPUS_TASK__DOC_WORLD_ANALYSIS_OPERATORS__CENTRALIT%209fe3882fa45441db93e18f4b0834565c.md)
    ...
    - Regression: 566 tests passed, 0 failed.[[2]](OPUS_TASK__DOC_WORLD_ANALYSIS_OPERATORS__CENTRALIT%209fe3882fa45441db93e18f4b0834565c.md)
...
- Реализация: добавлен чистый `trace()` оператор над doc-world (`operators/trace.js` + `worlds/documentation-world/*`) + CLI runner + examples JSON + тесты.[[1]](OPUS_TASK__DOC_WORLD_OPERATOR_02__TRACE%20d6012a84d24d4543972e49f756ab4144.md)
- Поведение: `trace(from, to)` возвращает либо кратчайший directed path, либо `GAP + candidate bridge` (suggestions-only; без канонизации).[[1]](OPUS_TASK__DOC_WORLD_OPERATOR_02__TRACE%20d6012a84d24d4543972e49f756ab4144.md)
...
- Верификация: 6 trace‑тестов, regression зелёный (572 passed, 0 failed). Engine не менялся.[[1]](OPUS_TASK__DOC_WORLD_OPERATOR_02__TRACE%20d6012a84d24d4543972e49f756ab4144.md)
...
- Верификация: добавлен `documentationWorldOperatorSupports.test.js` (8 тестов). Regression: 598 passed.[[1]](OPUS_TASK__OPERATOR_SUPPORTS_AND_RIVAL_PATHS_CHECK%20fc48dc9e8ca44b9c90c00beb16ac4972.md)
- Результат: на doc-world (51/108) Compare изначально не оправдан: rival paths = 0, rival bridge candidates = 0.[[1]](OPUS_TASK__OPERATOR_SUPPORTS_AND_RIVAL_PATHS_CHECK%20fc48dc9e8ca44b9c90c00beb16ac4972.md)
- Вывод: Compare требует измеримой неоднозначности.

### 2026-03-14 — Controlled ambiguity experiment: rival bridges (SUCCESS)

- Эксперимент: минимально усложнён doc-world вокруг паттерна `spec ↔ evidence`: +3 concept nodes, +8 edges (51/108 → 54/116), без новых типов.[[1]](OPUS_TASK__DOC_WORLD_CONTROLLED_AMBIGUITY_EXPERIME%20b261f9721e0147b7a71ded98cd9a10f7.md)
...
- Критерий оправданности Compare теперь формализован: Compare оправдан, когда `rankBridgeCandidates()` возвращает ≥2 кандидата с равным score или `findRivalTraces()` возвращает ≥2 shortest paths.[[1]](OPUS_TASK__DOC_WORLD_CONTROLLED_AMBIGUITY_EXPERIME%20b261f9721e0147b7a71ded98cd9a10f7.md)
- Engine unchanged; все тесты зелёные (599 passed).[[1]](OPUS_TASK__DOC_WORLD_CONTROLLED_AMBIGUITY_EXPERIME%20b261f9721e0147b7a71ded98cd9a10f7.md)
...
- Реализация: добавлен `compare()` оператор (diff/contrast, не adjudication) + CLI runner + examples + tests.[[1]](OPUS_TASK__DOC_WORLD_OPERATOR_03__COMPARE%20fd6f736cae784a99ac4443fad77e50a3.md)
- Поведение: `compare(G, from, to)` возвращает `PathSummary[]` (fingerprints) + `DiffSummary` (shared/unique/features/humanNotes).
- Верификация: 8 тестов Compare; regression green (per Opus report).[[1]]([OPUS_TASK__DOC_WORLD_OPERATOR_03__COMPARE](OPUS_TASK__DOC_WORLD_OPERATOR_03__COMPARE%20fd6f736cae784a99ac4443fad77e50a3.md))
...
- Реализация: добавлен `clusterRivalPaths()` (deterministic) + fingerprint key + derived labels; CompareResult расширен backward-compatible (`clusters`, `clusterCount`).[[1]]([OPUS_TASK__COMPARE_V0_1__CLUSTER_AND_LABEL_RIVAL_PATHS](OPUS_TASK__COMPARE_V0_1__CLUSTER_AND_LABEL_RIVAL_P%20905549bae49f45cf9c01313b83f94298.md))
...
- Валидация: прогнано 20 кейсов из 312 (spec|invariant → code_artifact), с покрытием разных подсетей и диапазоном paths 2–54.[[1]]([OPUS_TASK__VALIDATE_COMPARE_V0_1__ON_20_EMERGENT_CASES](OPUS_TASK__VALIDATE_COMPARE_V0_1__ON_20_EMERGENT_C%2062df679b93d1448782acad77bce575a0.md))
- Результаты:
...
- Вывод: fingerprint v0.1 достаточен; v0.1.1 refinement не требуется на текущем этапе.[[1]]([OPUS_TASK__VALIDATE_COMPARE_V0_1__ON_20_EMERGENT_CASES](OPUS_TASK__VALIDATE_COMPARE_V0_1__ON_20_EMERGENT_C%2062df679b93d1448782acad77bce575a0.md))
